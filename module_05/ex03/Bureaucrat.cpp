...
/*   Updated: 2021/09/17 17:17:21 by vlugand-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		Bureaucrat::executeForm(Form const &form)
{
	...
	{
		form.execute(*this);
		std::cout << this->_name << " executed " << form.getName() << " successfully." << std::endl;
	}
	catch(const Form::FormNotSignedException& e)
	{
		std::cerr << e.what() << std::endl;
		std::cout << form.getName() << " must be signed before execution." << std::endl;
	}
	catch(const Form::GradeTooLowException& e)
	{
		std::cerr << e.what() << std::endl;
		std::cout << this->_name << " is not qualified enough to execute " << form.getName();
		std::cout << ". Employee's current grade is " << this->_grade << " and grade " << form.getGradeExec() << " or higher is required for execution." << std::endl;
	}
	return ;
}
